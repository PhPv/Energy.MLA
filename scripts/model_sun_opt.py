...
def main_model(data_source=None, exp_name=None):
    ...
    clear_ml_metrics(model, X_test, y_test)
...
def read_clear_data():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X_train, y_train, test_size=0.05, random_state=42, shuffle=True
    )
...
def train_model(X_train, X_test, y_train, y_test):
    ...
    def objective(trial):
        ...
        params = {
            ...
            "lambda_l1": trial.suggest_loguniform("lambda_l1", 1e-8, 30.0),
            "lambda_l2": trial.suggest_loguniform("lambda_l2", 1e-8, 30.0),
        }

        model = lgb.LGBMRegressor(**params)
        scoring = dwmapeMetric()
        tscv = TimeSeriesSplit(n_splits=10) 
        cv_scores = cvs(model, X_train, y_train, cv=tscv, scoring=scoring)
    ...
    pickle.dump(
        model,
        open(f"models/{station_type}/{station_name}/{exp_name}model_{data_source}_lgbm.sav","wb",),)
...
def calc_metrics(model, X_test, y_test):
    ...
    df_res.to_excel(
        f"data/{station_type}/{station_name}/predict/train_predict_metrics_{data_source}.xlsx"
    )
...
main_model(data_source, exp_name)
